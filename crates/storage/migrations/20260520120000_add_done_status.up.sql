...
SELECT id, idea_id, author_user_id, body, created_at, updated_at
FROM comments;

CREATE TEMP TABLE comment_upvotes_backup AS
...
CREATE TABLE ideas_new (
  ...
  status TEXT NOT NULL DEFAULT 'open' CHECK(status IN ('open', 'inprogress', 'done', 'closed')),
  done_url TEXT,
  created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now')),
  updated_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now'))
);

INSERT INTO ideas_new (id, title, body, problem, author_user_id, status, done_url, created_at, updated_at)
SELECT id, title, body, problem, author_user_id, status, NULL, created_at, updated_at
FROM ideas;
...
INSERT OR IGNORE INTO comments (id, idea_id, author_user_id, body, created_at, updated_at)
SELECT b.id, b.idea_id, b.author_user_id, b.body, b.created_at, b.updated_at
FROM comments_backup b
JOIN ideas i ON i.id = b.idea_id
JOIN users u ON u.id = b.author_user_id;

INSERT OR IGNORE INTO comment_upvotes (comment_id, user_id, created_at)
SELECT b.comment_id, b.user_id, b.created_at
FROM comment_upvotes_backup b
JOIN comments c ON c.id = b.comment_id
JOIN users u ON u.id = b.user_id;

DROP TABLE upvotes_backup;
DROP TABLE comments_backup;
DROP TABLE comment_upvotes_backup;
